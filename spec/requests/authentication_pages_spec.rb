require 'spec_helper'

describe "AuthenticationPages" do
  subject { page }

  describe "signin page" do
    before { visit signin_path }

    it { should have_selector('h1',    text: 'Sign in') }
    it { should have_selector('title', text: 'Sign in') }
  end
  describe "signin" do
    before { visit signin_path }

    describe "with invalid information" do
      before { click_button "Sign in" }

      it { should have_selector('title', text: 'Sign in') }
      it { should have_selector('div.alert.alert-error', text: 'Invalid') }
    end

    describe "with valid information" do
      let(:geek) { FactoryGirl.create(:geek) }
      before do
        fill_in "Email",    with: geek.email
        fill_in "Password", with: geek.password
        click_button "Sign in"
      end

      it { should have_selector('title', text: geek.name) }
      it { should have_link('Users',    href: geeks_path) }
      it { should have_link('Profile', href: geek_path(geek)) }
      it { should have_link('Settings', href: edit_geek_path(geek)) }
      it { should have_link('Sign out', href: signout_path) }
      it { should_not have_link('Sign in', href: signin_path) }

      describe "after visiting another page" do
        before { click_link "Home" }
        it { should_not have_selector('div.alert.alert-error') }
      end
    end

  end
  describe "authorization" do

    describe "for non-signed-in users" do
      let(:geek) { FactoryGirl.create(:geek) }

      describe "in the Users controller" do
        describe "visiting the user index" do
          before { visit geeks_path }
          it { should have_selector('title', text: 'Sign in') }
        end

        describe "visiting the edit page" do
          before { visit edit_geek_path(geek) }
          it { should have_selector('title', text: 'Sign in') }
        end

        describe "submitting to the update action" do
          before { put geek_path(geek) }
          specify { response.should redirect_to(signin_path) }
        end
      end
      describe "when attempting to visit a protected page" do
        before do
          visit edit_geek_path(geek)
          fill_in "Email",    with: geek.email
          fill_in "Password", with: geek.password
          click_button "Sign in"
        end

        describe "after signing in" do

          it "should render the desired protected page" do
            page.should have_selector('title', text: 'Edit user')
          end
        end
      end
      describe "in the Plays controller" do

        describe "submitting to the create action" do
          before { post plays_path }
          specify { response.should redirect_to(signin_path) }
        end

        describe "submitting to the destroy action" do
          before { delete play_path(FactoryGirl.create(:play)) }
          specify { response.should redirect_to(signin_path) }
        end
      end
    end
    describe "as wrong user" do
      let(:geek) { FactoryGirl.create(:geek) }
      let(:wrong_user) { FactoryGirl.create(:geek, email: "wrong@example.com") }
      before { sign_in geek }

      describe "visiting Users#edit page" do
        before { visit edit_geek_path(wrong_user) }
        it { should_not have_selector('title', text: full_title('Edit user')) }
      end

      describe "submitting a PUT request to the Users#update action" do
        before { put geek_path(wrong_user) }
        specify { response.should redirect_to(root_path) }
      end
    end
  end

end
